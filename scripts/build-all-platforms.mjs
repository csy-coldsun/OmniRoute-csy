...
async function main() {
  ...
  log(`ℹ️  Node.js version: ${nodeVersion}`, colors.blue);

  if (!nodeVersion.match(/^v1[89]\.|^v2[0-3]\./)) {
    log(`⚠️  Warning: Node.js version should be >=18 and <24`, colors.yellow);
  }
  ...
  log("\n📦 Installing Electron dependencies...", colors.blue);
  if (!runCommand("npm install", electronDir)) {
    log("❌ Failed to install Electron dependencies", colors.red);
    process.exit(1);
  }
  ...
  // Step 5: Prepare Electron bundle
